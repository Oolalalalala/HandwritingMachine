 ...
 */
void TimedSyncDriver::startMove(long steps1, long steps2, long steps3){
...
void TimedSyncDriver::startMoveTimed(long steps1, long steps2, long duration){
    long steps[2] = {steps1, steps2};
    
    FOREACH_MOTOR(
        if (steps[i]){
            motors[i]->startMove(steps[i], duration);
            event_timers[i] = 1;
        } else {
         event_timers[i] = 0;
       }
